...
def communication_deg_detection(
    ...
    with anndata2ri.converter + ro.numpy2ri.converter + ro.pandas2ri.converter:
        ...
        summary_name = 'commot-'+database_name+'-sum-'+summary
        summary_abrv = 's' if summary == 'sender' else 'r'
        ...
        ro.r(f'assоRes = assoRes[assoRes[,"pvalue_1"] <= {deg_pvalue_cutoff}, ]')
        ro.r('oAsso <- order(assoRes[,"waldStat_1"], decreasing=TRUE)')
        ...
        string_cluster = (
            f'clusPat <- clusterExpressionPatterns(sce, nPoints = {n_points}, '
            f'verbose=TRUE, genes = rownames(assoRes)[oAsso][1:min({n_deg_genes},length(oAsso))], '
            'k0s=4:5, alphas=c(0.1))'
        )
        ro.r(string_cluster)
        ro.r('yhatScaled <- data.frame(clusPat$yhatScaled)')
